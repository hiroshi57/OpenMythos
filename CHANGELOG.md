...
> 注: [0.41.0] 以前のセクションは追記順のまま保存しており、一部バージョン順と一致しない。
> 0.38.0 が 2 回出現するのは当時の追補記載による (内容は別個)。

---

## [Unreleased] — 2026-07-04 全 Sprint 品質改革 (Fable 5)

### Phase 0: ベースライン + Sprint 72〜77 確定
- ruff 103 エラー → **0** (未使用 import 87 / 未使用変数 10 / f-string 6)
- Sprint 72〜77 の欠陥修正: `ImportError` 組み込み隠蔽 → `ImportRowError`、
  `CrowdLevel`/`CrowdSnapshot` 二重定義 → `PredictedCrowdLevel`/`CrowdObservation`、
  辞書順序依存分類器 → 明示 if/elif、`map_animator` 非東京都市の ID/都市ラベル修正、
  `WEIGHT_CROWD_AVOIDANCE` を DEFAULT_WEIGHT_PRESETS に登録
- 全 4425 テスト実行 triage: 失敗 5 + エラー 13 を修正

### Phase 1: 品質基盤
- CI (`ci.yml`): テスト 2 ファイル → **全 88 ファイル実行**、`serve/` を lint + coverage 対象化
- pyproject: `[tool.pytest.ini_options]` + マーカー登録、バージョン 0.57.0 → **0.80.0**
- バージョン一元化: `open_mythos.__version__` を単一ソースに (api.py が参照)

### Phase 2: 構造改革 — serve/api.py ルーター分割
- **9264 行モノリス → ドメイン別 `serve/routers/`** (map / ads / llm_services /
  skills_integrations / growing_ai)。ルート数 305 を分割前後で完全保存
- モデル状態 `_State` → `serve/state.py` (`AppState`) に共有化
- 推論ループ定数 (DEFAULT_LOOPS/MAX_LOOPS/TASK_LOOPS) を `serve/state.py` に一元化
- `serve/ab_router.py` / `sla_router.py` / `monitor.py` は**スタンドアロン構成として維持**
  (monitor は docker-compose の独立サービス PORT 8002、他はテスト現役)

---

## [0.47.0 〜 0.80.0] — 2026-06-07 〜 2026-07-03 (Sprint 44〜77 要約)

> 個別エントリ未記載期間の一括要約。詳細は `Plans.md` の Sprint 全サマリー参照。

| Ver | Sprint | テーマ |
|-----|--------|--------|
| 0.47.0 | 44 | Vector DB 統合 + Instructor 構造化出力 |
| 0.48.0 | 45 | HuggingFace Hub 統合 |
| 0.49.0 | 46 | 推論バックエンド統合 |
| 0.50.0 | 47 | 研究ツール統合 |
| 0.51.0 | 48 | マルチモーダル統合 |
| 0.52.0 | 49 | 訓練最適化統合 |
| 0.53.0 | 50 | エージェントフレームワーク強化 |
| 0.54.0 | 51 | データ・検索ツール統合 |
| 0.55.0 | 52 | DevOps・クラウド統合 |
| 0.56.0 | 53 | セキュリティ統合 |
| 0.57.0 | 54 | OpenAI Assistants API 互換 |
| 0.58.0 | 55 | ストリーミング & SSE 応答 |
| 0.59.0 | 56 | マルチプロバイダー LLM |
| 0.60.0 | 57 | LLM 評価フレームワーク |
| 0.61.0 | 58 | LLMO ダッシュボード・CEP 管理・競合分析 |
| 0.62.0 | 59 | 自律脆弱性スキャン (harness 移植) |
| 0.63.0 | 60 | 広告キャンペーン管理 |
| 0.64.0 | 61 | 日本語対応トークナイザー |
| 0.65.0 | 62 | LLM コピー生成強化 |
| 0.66.0 | 63 | A/B テスト + 分析 DB + 形態素解析 |
| 0.67.0 | 64 | A/B+分析 API + 形態素連携 + 予算最適化 |
| 0.68.0 | 65 | Fusion マルチモデル融合 |
| 0.69.0 | 66 | Fusion ストリーミング + A/B 予算連携 + 異常検知 |
| 0.70.0 | 67 | 異常検知自動予算停止 + Fusion キャッシュ + 統合ダッシュボード |
| 0.71.0 | 68 | セキュリティインテリジェンス + リスクカテゴリ分類 |
| 0.72.0 | 69 | 時系列予測 TimesFM + マルチモデル |
| 0.73.0 | 70 | 予測アラート統合 + レポート Webhook + NLQ エージェント |
| 0.74.0 | 71 | 主要都市地図ビジュアライザ (Metro 地下断面) |
| 0.75.0 | 72 | 地図拡張 比較/編集/レポート |
| 0.76.0 | 73 | 地図アニメーション/経路探索/インポート |
| 0.77.0 | 74 | 混雑シミュレーション/アクセシビリティ/地下水位 |
| 0.78.0 | 75 | 環境センサー/乗り換え最適化/インフラダッシュボード |
| 0.79.0 | 76 | 交通量分析/エネルギーモニタリング/群衆予測 |
| 0.80.0 | 77 | 災害アラート/水質モニタリング/騒音マッピング |

> 0.42.0〜0.46.0 (Sprint 39〜43: ショーケースダッシュボード/completions/
> embeddings/Hermes Layer2 ほか) は個別記録なし。
