    ...
    def __init__(self, path: Path, max_length: int = 512):
        ...
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                self.records.append({"input_ids": ids, "labels": lbl,
...
def train(args):
    ...
    print(f"[finetune] task={args.task} device={device} loops={n_loops} lr={lr} epochs={epochs}")
    ...
    scheduler   = get_cosine_schedule_with_warmup(
        optimizer, num_warmup_steps=max(1, total_steps // 20),
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch_data in enumerate(train_loader):
            ...
            loss = (loss_per_token * w_expanded * valid.float()).sum() / valid.float().sum().clamp(min=1)
    ...
    print(f"推論サーバーで使うには MODEL_CHECKPOINT={out_dir / 'best.pt'} を設定してください。")
...
def _small_config() -> MythosConfig:
    return MythosConfig(
        vocab_size=50257, dim=256, n_heads=8, n_kv_heads=2,
        max_seq_len=512, max_loop_iters=16, prelude_layers=1, coda_layers=1,
        attn_type="gqa", n_experts=8, n_shared_experts=1,
        n_experts_per_tok=2, expert_dim=128, act_threshold=0.99,
        lora_rank=8, kv_lora_rank=64, q_lora_rank=128,
...
def main():
    ...
    parser.add_argument("--task",       default="all",
                        choices=["content_quality","ad_performance",
                                 "persona_segment","market_research","all"])
    parser.add_argument("--data-dir",   required=True, help="前処理済みJSONLのディレクトリ")
    parser.add_argument("--checkpoint", default="",    help="初期チェックポイント（省略=ランダム初期化）")
