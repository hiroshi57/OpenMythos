...
class PredictedCrowdLevel(str, Enum):
    SPARSE  = "sparse"   # 閑散
    NORMAL  = "normal"   # 通常
    CROWDED = "crowded"  # 混雑
    PACKED  = "packed"   # 超混雑
...
# PredictedCrowdLevel 分類しきい値（人/時）
_LEVEL_THRESHOLDS: Dict[PredictedCrowdLevel, float] = {
    PredictedCrowdLevel.PACKED:  3000.0,
    PredictedCrowdLevel.CROWDED: 1500.0,
    PredictedCrowdLevel.NORMAL:   500.0,
    PredictedCrowdLevel.SPARSE:     0.0,
}


def _classify_crowd(count: float) -> PredictedCrowdLevel:
    for level, threshold in _LEVEL_THRESHOLDS.items():
        if count >= threshold:
            return level
    return PredictedCrowdLevel.SPARSE
...
class CrowdObservation:
    ...
    @property
    def level(self) -> PredictedCrowdLevel:
...
class CrowdPrediction:
    ...
    predicted_level: PredictedCrowdLevel
    hour_scale: float
    weather_scale: float
    event_boost: float
...
class CrowdStore:
    ...
    def __init__(self) -> None:
        self._snapshots: Dict[str, CrowdObservation] = {}

    def add(self, snap: CrowdObservation) -> CrowdObservation:
        self._snapshots[snap.snapshot_id] = snap
        return snap

    def get(self, snapshot_id: str) -> Optional[CrowdObservation]:
        return self._snapshots.get(snapshot_id)

    def list_all(self) -> List[CrowdObservation]:
        return list(self._snapshots.values())

    def list_by_city(self, city: str) -> List[CrowdObservation]:
        return [s for s in self._snapshots.values() if s.city == city]

    def list_by_spot(self, spot_name: str, city: str) -> List[CrowdObservation]:
