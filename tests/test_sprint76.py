...
class TestTrafficStore:
    def test_add_and_get(self):
        ...
        analyzer.add_segment("s1", "国道1号", "Tokyo", 1200, 45.0, 15.0)
...
class TestEnergyStore:
    def test_add_and_get(self):
        ...
        monitor.add_reading("r1", "fac1", "Tokyo", EnergyType.ELECTRICITY, 120.0)
...
from open_mythos.skills.crowd_predictor import (
    CrowdPredictor, CrowdStore, PredictedCrowdLevel, WeatherCondition, EventType,
    _classify_crowd,
)
...
class TestPredictedCrowdLevel:
    def test_sparse(self):
        assert _classify_crowd(100.0) == PredictedCrowdLevel.SPARSE

    def test_normal(self):
        assert _classify_crowd(700.0) == PredictedCrowdLevel.NORMAL

    def test_crowded(self):
        assert _classify_crowd(2000.0) == PredictedCrowdLevel.CROWDED

    def test_packed(self):
        assert _classify_crowd(4000.0) == PredictedCrowdLevel.PACKED
...
class TestCrowdSnapshot:
    def test_level_crowded(self):
        ...
        assert snap.level == PredictedCrowdLevel.CROWDED
...
class TestCrowdPredictor:
    def test_predict_returns_result(self):
        ...
        assert result.predicted_level in list(PredictedCrowdLevel)
    ...
    def test_daily_forecast_rush_packed(self):
        ...
        assert any(f.predicted_level in (PredictedCrowdLevel.CROWDED, PredictedCrowdLevel.PACKED) for f in rush)
