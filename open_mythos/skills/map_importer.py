...
class StationCSVImporter:
    ...
    def import_csv(self, csv_text: str) -> ImportResult:
        ...
        if missing:
            result.errors.append(ImportRowError(0, f"必須カラム不足: {missing}"))
            return result
        ...
        for i, row in enumerate(reader, start=1):
            ...
            try:
                ...
                result.errors.append(ImportRowError(i, str(e), str(row)))
...
class GeoJSONImporter:
    ...
    def import_geojson(self, geojson_text: str) -> ImportResult:
        ...
        try:
            ...
            result.errors.append(ImportRowError(0, f"JSON parse error: {e}"))
            return result

        if data.get("type") != "FeatureCollection":
            result.errors.append(ImportRowError(0, "type が FeatureCollection ではありません"))
            return result
        ...
        for i, feat in enumerate(features, start=1):
            ...
            try:
                ...
                if len(coords) < 2:
                    result.errors.append(ImportRowError(i, "coordinates が不足しています"))
                    continue
                ...
                if missing:
                    result.errors.append(ImportRowError(i, f"properties 不足: {missing}"))
                    continue
                ...
                result.errors.append(ImportRowError(i, str(e)))
