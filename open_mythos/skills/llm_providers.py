...
モデルティア (Sprint 61):
  ...
  ⚠️ 「Fable 5」「Mythos 5」は本プロジェクト (OpenMythos) 独自の内部呼称であり、
     Anthropic 公式のモデル名・モデル文字列ではない。各ティアは上記のとおり
     実在する Anthropic API モデル ID (claude-sonnet-4-5 / claude-opus-4) に
     マッピングされる薄いエイリアスにすぎない。`claude-fable-5` のような
     モデル文字列は存在しない。
...
class ClaudeModelTier(str, Enum):
    ...
    注意:
        「Fable 5」「Mythos 5」「Haiku 5」は OpenMythos 独自の内部呼称 (エイリアス)
        であり、Anthropic 公式の製品名・モデル文字列ではない。enum の値こそが
        実際に API へ渡される正規のモデル ID であり、tier_label が返す
        "Fable 5 (Balanced)" 等の表示名はあくまで本プロジェクト内の便宜的ラベル
        である。`claude-fable-5` / `claude-mythos-5` のようなモデル文字列は
        実在しないため、外部の説明やリクエストで使用してはならない。

    Attributes:
        HAIKU_5:   高速・低コスト (既存デフォルト) → claude-haiku-4-5
        FABLE_5:   バランス・一般向けの呼称        → claude-sonnet-4-5
        MYTHOS_5:  最高性能・サイバー防衛向けの呼称 → claude-opus-4
    """
