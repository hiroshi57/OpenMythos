...
def mythos_7b() -> MythosConfig:
    """7B parameter config. Efficient reasoning model. dim=3584, 128 experts, 20 loop iters, 8k context.

    Positioned between ``mythos_3b`` and ``mythos_10b``.  Uses the same MLA
    attention as all other variants and targets ~7B total parameters (dominated
    by the MoE expert pool: 128 experts × 3 matrices × 3584 × 5632 ≈ 7.7B).
    """
    return MythosConfig(
        vocab_size=32000,
        dim=3584,
