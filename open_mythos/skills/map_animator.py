...
class SurveyDataset:
    ...
    def build(cls, city: CityName) -> List[SurveySnapshot]:
        ...
        # 他都市は Tokyo の層構成を流用し、都市名・レイヤー ID を差し替える
        snapshots = cls.build_tokyo()
        for s in snapshots:
            s.city = city
            for layer in s.layers:
                layer.city = city
                layer.id = f"{city.value}-{s.year}-{layer.layer_type.value}"
        return snapshots
...
class MapAnimator:
    ...
    def _render_year_labels(
        ...
        # SVG text の内容を animate で変えるのは非標準のため、
        # 固定ラベルを複数配置し animate visibility で切替する方式を採用
        root.append(lbl)
